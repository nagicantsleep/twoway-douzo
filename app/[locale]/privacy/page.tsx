  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 800, margin: '0 auto', padding: '60px 24px 80px', color: 'var(--tx-1)', lineHeight: 1.8 }}>
        ...
        <h2 style={{ fontSize: 18, marginTop: 32, marginBottom: 12 }}>{t('section8.title')}</h2>
        <p>{t('section8.body')}</p>

        <p style={{ marginTop: 48, fontSize: 12, color: 'var(--tx-3)' }}>
          <a href="/terms" style={{ color: 'var(--ac)' }}>{t('footer.termsLink')}</a> · <a href="/" style={{ color: 'var(--ac)' }}>{t('footer.homeLink')}</a>
        </p>
      </main>
    </>
  );
